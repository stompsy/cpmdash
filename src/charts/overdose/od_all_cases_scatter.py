...
def create_gantt_chart(df, jail_periods_df, y_categories, color_map):
    ...
    patient_ranges = df.groupby("merged_label", observed=False)["od_date"].agg(["min", "max"]).reset_index()
    
    # Create base timeline bars for each patient
    for idx, row in patient_ranges.iterrows():
...
def create_swimlane_chart(df, jail_periods_df, color_map, age_map, sex_map):
    ...
    fig = style_plotly_layout(
        ...
        scroll_zoom=False,
        show_legend=False,
        margin=dict(t=60, l=50, r=20, b=20),
        height=800
    )
...
def create_calendar_heatmap(df, y_categories):
    ...
    heatmap_data = df_copy.groupby(["merged_label", "year_month"], observed=True).size().reset_index(name="od_count")
    
    # Pivot for heatmap
    heatmap_pivot = heatmap_data.pivot(index="merged_label", columns="year_month", values="od_count").fillna(0)
